...
def askfiledir(titlename, filetype, path=" "):
    ...
    try:
        ...
        filedir = filedialog.askopenfilename(
            initialdir=cdir, title=titlename, filetype=filetype
        )
        root.destroy()
...
def backup(dir1, file1):
    ...
    logging.info(
        "backup config file created. [{}.backup.{}]".format(file1, now_time)
    )
...
if __name__ == "__main__":
    backup(
        "C:\\Users\\stopc\\AppData\\LocalLow\\Ludeon Studios\\RimWorld by Ludeon Studios\\Config",
        "ModsConfig.xml",
    )
