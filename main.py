...
)  # TODO change to more readable format.
...
def main():
        ...
        if a.isalpha():
            if a.lower() == "y":
                ...
                for x in Modmanager.Mod.MODs:
                    Modmanager.Mod.ActiveModlist.append(
                        x.MODkey
                    )  # 로드된 모드를 전부 활성화 리스트에 집어넣음
    ...
    RWmanager.backup(
        Modmanager.ModBase.Configxmlfolderpath, "ModsConfig.xml"
    )  # backup file
    Modmanager.update_config(
        Modmanager.ModBase.Configxmlfolderpath, mods
    )  # write ModsConfig.xml and save.
    ...
    nMods = (
        Modmanager.Mod.list2 + Modmanager.Mod.list4
    )  # should move to ModManager class method
    if nMods != []:  # should
    ...
    print(
        "log will collect :\n {}".format(logcollect)
    )  # let users know what data will be upload to github gist.
